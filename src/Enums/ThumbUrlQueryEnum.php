...
enum ThumbUrlQueryEnum: string
{
    ...
    {
        $allowedValues = self::getConfAllowed($thumbParam);
        // no limit size @warning on security, better set it to no flood by file the server
        if (!$allowedValues && ($thumbParam == self::WIDTH->value || $thumbParam == self::HEIGHT->value)) {
            return  true;
        }
